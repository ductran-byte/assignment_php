...
$urlhientai = $_SERVER['REQUEST_URI'];
...
if($requestid > 1){
 ...
 $aiduoctien = $truyvan1['uid'];
  ...
  if($baonhieu == '1'){
   ...
   $soxudc = $value_receive * $khuyenmai;
   mysqli_query($ketnoi, "UPDATE `player` SET `coin` = `coin` + '$soxudc' ,`coin_old` = `coin_old` + '$soxudc' , `tongnap` = `tongnap` + '$soxudc' , `tongthang` = `tongthang` + '$soxudc' where `username` = '$aiduoctien' ") or ('Ko cong tien');
